...
| `templates/` | Starter files consumers copy verbatim: the `AGENTS.md` seed, the one-line `CLAUDE.md` shim, the per-repo sync stub, and the `wiki/` starter. |
| `templates/workflows-repo/sync-shared-rules.yml` | Bootstrap copy of the reusable sync workflow; its canonical home is `JINGBANZ/workflows/.github/workflows/sync-shared-rules.yml`. |
| `.github/workflows/sync-shared-rules.yml` | This repo's own sync stub — keeps the marker block in `templates/AGENTS.md` in step with `shared-rules.md`. |
...
- When you change `shared-rules.md`, update the marker block in `templates/AGENTS.md` **in the same
  PR** (copy the file content verbatim between the markers). The sync workflow corrects drift, but
  don't ship it knowingly.
- The marker format (`<!-- shared-rules:begin -->` / `<!-- shared-rules:end -->`) is coupled to the
  splice step in the reusable workflow — change them together or not at all.
